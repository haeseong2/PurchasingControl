...
public class UserDAO {
    ...
    /**
     * メソッド名: insertUser
     * 役割: ユーザー情報を USER_INFO テーブルに新規登録する。
     * 作成者: [イ・グァンフン]
     * 作成日: [2025年０４月０３日]
     */
    ...
    /**
     * メソッド名: login
     * 役割: ユーザーIDとパスワードを用いてログイン認証を行う。
     *       USER_INFO テーブルに該当するレコードが存在するかを確認する。
     * 作成者: [イ・グァンフン]
     * 作成日: [2025年０４月０３日]
     */
    ...
    /**
     * メソッド名: getUserByIdAndPw
     * 役割: 指定されたユーザーIDとパスワードに一致するユーザーのステータス（USER_STATUS）を取得する。
     *       ユーザー情報が一致すれば、UserDTO オブジェクトにステータスを設定して返す。
     * 作成者: [イ・グァンフン]
     * 作成日: [2025年０４月０３日]
     */
