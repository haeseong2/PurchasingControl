...
<!-- 요청 실패 모달 -->
<div class="modal fade" id="RequestFailModal" tabindex="-1" aria-labelledby="RequestFailModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="requestFailModalLabel">로그인 실패</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        형식이 맞지 않습니다.
...
<script>
  ...
  <% if (request.getAttribute("requestSuccess") != null && (boolean) request.getAttribute("requestSuccess")) { %>
    $(document).ready(function() {
        $('#RequestSuccessModal').modal('show'); // 회원가입 성공 모달
    });
  <% } %>
  ...
  <% if (request.getAttribute("requestFail") != null && (boolean) request.getAttribute("requestFail")) { %>
    $(document).ready(function() {
        $('#RequestFailModal').modal('show'); // 로그인 실패 모달
    });
  <% } %>
</script>
